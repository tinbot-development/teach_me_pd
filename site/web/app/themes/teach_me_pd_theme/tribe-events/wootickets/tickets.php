...
<form action="<?php echo esc_url( $woocommerce->cart->get_cart_url() ) ?>" class="cart" method="post" enctype='multipart/form-data'>
    ...
    if(count($tickets) > 1):
          foreach ( $tickets as $key => $ticket ) {
            ...
            if ( ( empty( $start_date ) || time() > $start_date ) && ( empty( $end_date ) || time() < $end_date ) ) {
              ...
              echo '<input type="checkbox" name="'.$ticket->ID.'" id="'.$ticket->ID.'" class="ticket-selector">';
              echo '<div class="hidden input-holder">';
              ...
              echo '<td colspan="4" class="hidden tickets_description">';
    ...
    /* If Ticket created is equal to 1 */
    else:
  ...
  <script type="text/javascript">
    jQuery(function ($) {
      ...
      /*Ticket Selector*/
      $('.ticket-selector').on("click", function(){
          if($(this).attr('checked') == 'checked') {
            $(this).siblings('div.input-holder').find('.quantity').find('input.qty.text').val(1);
          } else {
            $(this).siblings('div.input-holder').find('.quantity').find('input.qty.text').val(0);
          }
      });
    })
  </script>
