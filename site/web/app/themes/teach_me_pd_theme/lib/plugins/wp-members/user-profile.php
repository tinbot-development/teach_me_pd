...
/** Override WP Members Default Actions */
...
add_action( 'show_user_profile', 'pb_wpmem_admin_fields' );
add_action( 'edit_user_profile', 'pb_wpmem_admin_fields' );
add_action( 'profile_update',    'pb_wpmem_admin_update' );
 ...
 */
function pb_wpmem_admin_fields() {
  ...
  <h3 class="<?php echo (in_array('subscriber',$user->roles))? '' :'hidden'; ?>"><?php
    /**
...
<table class="form-table <?php echo (in_array('subscriber',$user->roles))? '' :'hidden'; ?>">
  <?php
  // Get fields.
 ...
 * @since 2.1
 */
function pb_wpmem_admin_update() {
